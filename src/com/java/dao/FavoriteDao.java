...
public class FavoriteDao extends AbstractEntityDao<Favorite> {
	...
	public List<FavoriteUserReport> reportFavoriteUsersByVideo(String videoId){
		String jpql = "select new com.java.domain.FavoriteUserReport(f.user.username, f.user.fullname, "
	...
	public List<FavoriteReport> reportFavoritesByVideos() {
		String jpql = "select new com.java.domain.FavoriteReport(f.video.title, count(f), min(f.likedDate), max(f.likedDate))"
